...
var MoLsp = (function () {
    ...
    function createSpform(form) {
        return new Spform(form);
    }
    ...
    function createEnv(name, subst) {
        return new Set(name, subst);
    }

    function createLambda(form, args, env) {
        return new Lambda(form, args, env);
    }

    function createHtmldom(htag, dom) {
        return new Htmldom(htag, dom);
    }

    function createHtmldoc(doc) {
        return new Htmldoc(doc);
    }

    function createAry(ary) {
        return new Ary(ary);
    }

    // ----------------------------------------------------------------------------------
    function numberp(x) {
        return (x.tag === "num") ? T : Nil;
    }

    function consp(x) {
        return (x.tag === "cell") ? T : Nil;
    }

    function stringp(x) {
        return (x.tag === "string") ? T : Nil;
    }

    function regexpp(x) {
        return (x.tag === "regexp") ? T : Nil;
    }
    ...
    function valToBool(val) {
        if (val.tag === "num") {
            return (val.num === 0) ? Nil : T;
        }
        return (val === Nil) ? Nil : T;
    }

    function car(x) {
        return (x === Nil) ? Nil : x.car;
    }

    function cdr(x) {
        return (x === Nil) ? Nil : x.cdr;
    }
    ...
    function nullp(x) {
        return (x === Nil || (car(x) === Nil && cdr(x) === Nil)) ? T : Nil;
    }

    function length(lst) {
        return (nullp(lst) === T) ? 0 : 1 + length(cdr(lst));
    }

    function append(lst1, lst2) {
        return (nullp(lst1) === T) ? lst2 : cons(car(lst1), append(cdr(lst1), lst2));
    }
    ...
    function list(lst) {
        return (lst === Nil) ? lst : cons(car(lst), list(cdr(lst)));
    }
    ...
    function built_quote(args, env) {
        return car(args);
    }

    function built_car(args) {
        return caar(args);
    }

    function built_cdr(args) {
        return cdar(args);
    }

    function built_cons(args) {
        return cons(car(args), car(cdr(args)));
    }

    function built_list(args) {
        return list(args);
    }
    ...
    function built_atom(s) {
        return (car(s).tag !== "cell") ? T : Nil;
    }
    ...
    function built_consp(args) {
        return consp(car(args));
    }

    function built_zerop(args) {
        return zerop(car(args));
    }

    function built_stringp(s) {
        return stringp(car(s));
    }

    function built_regexpp(s) {
        return regexpp(car(s));
    }

    function built_symbolp(s) {
        return symbolp(car(s));
    }
    ...
    function built_cond(args, env) {
        while (args != Nil) {
            if (sexpEval(caar(args), env) !== Nil) {
                return built_progn(cdar(args), env);
            }
            args = cdr(args);
        }
        return Nil;
    ...
    function Build_search_table_rx(args) {
        ...
        for (var i = 0; i < tableObjs.length; i++) {
            for (var current = car(args); current !== Nil; current = cdr(current)) {
                if (matchTr_rx(car(current), tableObjs[i]) !== Nil) {
                    mLevel++;
                }
            }
            if (mLevel === length(car(args))) {
                return createHtmldom("table", tableObjs[i]);
            }
            mLevel = 0;
        }
        return Nil;
    ...
    function Build_match_table_rx(args) {
        ...
        for (var current = model; current !== Nil; current = cdr(current)) {
            if (matchTr_rx(car(current), tableObj) !== Nil) {
                mLevel++;
            }
        }
    ...
    var sexpParse = function (src) {
        ...
        var parse = function (ary) {
            return parseAryToObj(ary, 0, ary.length - 1);
        };
    ...
    var self = {
        ...
        //function
        "parse":function (src) {
            return sexpParse(src);
        },
        "eval":function (sObj) {
            return sexpEval(sObj, this.GlobalEnv);
        },
        "output":function (sObj) {
            return outputObjToStr(sObj);
        },
        "run":function (src) {
